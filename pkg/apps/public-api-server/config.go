...
func init() {
	...
	flag.StringVar(&config.DB, "database DSN", "user=postgres password=q dbname=postgres sslmode=disable", "DSN for database")
	flag.StringVar(&config.APIPrefix, "prefix URL for API", "/api/v1", "URL for requests for API")
	flag.StringVar(&config.AllowedIP, "allowed IP", "http://localhost:3000", "IP for CORS")
